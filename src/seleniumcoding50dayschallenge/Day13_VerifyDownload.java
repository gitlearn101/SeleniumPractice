...
public class Day13_VerifyDownload {

	public static WebDriver driver;
	public static String pdfUrl = null;
	public static String outputFilePath = System.getProperty("user.dir") + "\\Downloads";
	...
	void run() {
		...
		WebElement downloadLink = driver.findElement(By.linkText("Download the printable PDF of Selenium cheat sheet"));

		String mainWindowHandle = driver.getWindowHandle();

		downloadLink.click();

		// wait until new window is ready
		WebDriverWait waitNewWindowReady = new WebDriverWait(driver, Duration.ofSeconds(10));
		waitNewWindowReady.until(ExpectedConditions.numberOfWindowsToBe(2));

		// switch to new window
		Set<String> allWindowHandles = driver.getWindowHandles();
		System.out.println("Nos of windows >>" + allWindowHandles.size());

		for (String handle : allWindowHandles) {
			if (!handle.equals(mainWindowHandle)) {
				driver.switchTo().window(handle);

				waitNewWindowReady.until(driver -> {
		...
		try {
			...
			try (InputStream inputStream = connection.getInputStream();
					FileOutputStream outputStream = new FileOutputStream(
							outputFilePath + File.separator + pdfFileName)) {
